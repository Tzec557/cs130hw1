...
{
    double u_scaled = std::floor(uv[0] * width);
    double v_scaled = std::floor(uv[1] * height);

    int i = wrap(static_cast<int>(u_scaled), width);
    int j = wrap(static_cast<int>(v_scaled), height);

    i = std::max(0, std::min(width - 1, i));
    j = std::max(0, std::min(height - 1, j));
   
    Pixel p = data[j * width + i];
    return From_Pixel(p);
}
